 ...
 *
 * Takes in a numeric key and an input phrase an performs the Caesars Cipher.
 */
...
{	
	...
	else
	{
		...
		printf("The rotation number is %i\n", key);
	}
	...
	for (int i = 0, n = strlen(plaintext); i < n; i++)
	{
		...
		{
			plaintext[i] = ((int)plaintext[i] + key); // something strange happened, ascii went negative
